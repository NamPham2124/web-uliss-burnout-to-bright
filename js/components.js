...
window.COMPONENTS = {
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex items-center justify-between h-16">
            ...
            <div class="flex items-center space-x-1.5 sm:space-x-2.5">
              ...
              <!-- Sound / BGM Button -->
              <button onclick="APP.toggleAudioPlay()" title="Bật/Tắt nhạc nền violin & thư giãn" 
                      class="px-2 sm:px-2.5 py-1.5 rounded-xl border flex items-center space-x-1.5 text-xs font-bold transition-all ${isPlaying ? 'bg-emerald-100 dark:bg-emerald-950 border-emerald-300 dark:border-emerald-700 text-emerald-800 dark:text-emerald-300 ring-2 ring-emerald-300' : 'bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100'}">
                <i class="fas ${isPlaying ? 'fa-music text-emerald-600 animate-pulse' : 'fa-volume-xmark text-slate-400'}"></i>
  ...
  renderFloatingAudioWidget: function(audioState) {
    const isPlaying = audioState && audioState.isPlaying;
    const currentTrack = audioState ? audioState.currentTrack : 'senbonzakura';
    const trackInfo = audioState?.trackInfo || { name: 'Senbonzakura (Violin Cover)', artist: 'Lindsey Stirling' };
    const volume = audioState ? Math.round(audioState.volume * 100) : 35;
    const isMinimized = window.APP?.state?.audioWidgetMinimized || false;

    if (isMinimized) {
      return `
        <div id="floatingAudioPlayer" class="fixed bottom-4 right-4 z-40">
          <button onclick="APP.state.audioWidgetMinimized = false; APP.render();" 
                  title="Mở trình phát nhạc (${trackInfo.name})"
                  class="w-12 h-12 rounded-full bg-gradient-to-tr from-emerald-600 to-teal-500 hover:from-emerald-700 hover:to-teal-600 text-white shadow-xl flex items-center justify-center transition-all transform hover:scale-105 active:scale-95 border-2 border-white relative group">
            <i class="fas ${isPlaying ? 'fa-music animate-bounce' : 'fa-compact-disc'} text-base"></i>
            ${isPlaying ? `<span class="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-rose-500 ring-2 ring-white animate-pulse"></span>` : ''}
          </button>
        </div>
      `;
    }
    ...
    return `
      <div id="floatingAudioPlayer" class="fixed bottom-4 right-4 z-40 glass-card rounded-2xl p-3 border-2 border-emerald-300/80 bg-white/95 dark:bg-slate-900/95 shadow-2xl flex flex-col space-y-2 transition-all duration-300 max-w-[320px] backdrop-blur-md">
        ...
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-1.5">
          <div class="flex items-center space-x-1.5 overflow-hidden">
            <span class="w-6 h-6 rounded-lg ${isPlaying ? 'bg-emerald-500 text-white animate-pulse' : 'bg-slate-100 text-slate-500'} flex items-center justify-center text-xs flex-shrink-0">
              <i class="fas ${isPlaying ? 'fa-music' : 'fa-volume-xmark'}"></i>
            </span>
            <div class="truncate">
              <div class="text-[11px] font-extrabold text-slate-900 dark:text-white truncate" title="${trackInfo.name}">
                ${trackInfo.name}
              </div>
              <div class="text-[9px] text-slate-500 dark:text-slate-400 -mt-0.5 truncate">
                ${trackInfo.artist || 'Nhạc nền'} • <span class="${isPlaying ? 'text-emerald-600 dark:text-emerald-400 font-bold' : 'text-slate-400'}">${isPlaying ? 'Đang phát' : 'Tạm dừng'}</span>
              </div>
            </div>
          </div>

          <button onclick="APP.state.audioWidgetMinimized = true; APP.render();" 
                  title="Thu nhỏ trình phát nhạc"
                  class="text-slate-400 hover:text-slate-600 w-5 h-5 flex items-center justify-center text-xs rounded hover:bg-slate-100 transition-colors ml-1 flex-shrink-0">
        ...
        <div class="flex items-center justify-between gap-2 pt-0.5">
          <div class="flex items-center space-x-1">
            <button onclick="APP.prevAudioTrack()" title="Bài trước" class="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 flex items-center justify-center text-xs transition-all active:scale-90">
              <i class="fas fa-backward-step"></i>
            </button>
            <button onclick="APP.toggleAudioPlay()" title="${isPlaying ? 'Tạm dừng' : 'Phát nhạc'}" class="w-9 h-9 rounded-xl ${isPlaying ? 'bg-emerald-600 hover:bg-emerald-700 text-white shadow-emerald-200' : 'bg-slate-900 hover:bg-black text-white'} flex items-center justify-center shadow-md text-sm transition-all active:scale-95">
              <i class="fas ${isPlaying ? 'fa-pause' : 'fa-play'}"></i>
            </button>
            <button onclick="APP.nextAudioTrack()" title="Bài kế tiếp" class="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 flex items-center justify-center text-xs transition-all active:scale-90">
              <i class="fas fa-forward-step"></i>
            </button>
          </div>

          <!-- Volume slider -->
          <div class="flex items-center space-x-1.5 flex-1 justify-end">
            <i class="fas ${volume === 0 ? 'fa-volume-xmark text-slate-400' : 'fa-volume-low text-emerald-600'} text-[10px]"></i>
            <input type="range" min="0" max="100" value="${volume}" oninput="APP.setAudioVolume(this.value / 100)" class="w-16 h-1 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-emerald-600" title="Âm lượng (${volume}%)">
          </div>
        </div>

        <!-- Track Selector -->
        <div>
          <select onchange="APP.changeAudioTrack(this.value)" class="w-full text-[10px] font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-xl px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer">
            <optgroup label="🎻 Nhạc Violin Tiếp Năng Lượng">
              <option value="senbonzakura" ${currentTrack === 'senbonzakura' ? 'selected' : ''}>🎻 Senbonzakura (Violin Cover)</option>
              <option value="counting_stars" ${currentTrack === 'counting_stars' ? 'selected' : ''}>🎻 Counting Stars (Violin Cover)</option>
              <option value="we_dont_talk_anymore" ${currentTrack === 'we_dont_talk_anymore' ? 'selected' : ''}>🎻 We Don't Talk Anymore (Violin)</option>
            </optgroup>
            <optgroup label="🌿 Nhạc Thư Giãn Não Bộ (Ambient)">
              <option value="healing_piano" ${currentTrack === 'healing_piano' ? 'selected' : ''}>🎹 Piano Dịu Êm & Chữa Lành</option>
              <option value="solfeggio_432" ${currentTrack === 'solfeggio_432' ? 'selected' : ''}>🧘 Tần số 432Hz Thiền Định</option>
              <option value="zen_nature" ${currentTrack === 'zen_nature' ? 'selected' : ''}>🍃 Tiếng Suối Nguồn & Rừng Xanh</option>
            </optgroup>
          </select>
        </div>
      </div>
